...
const BB_GIST_ID = "58f5826f24892d2d5121bc180755dabd";
// Token encrypted with BB_PASSWORD via AES-GCM (safe to hardcode in source)
const BB_ENC_TOKEN_HARDCODED = "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";

/* ── Storage keys ─────────────────────────────────────────────────────────── */
const BB_KEY_PROGRESS = "bb_progress_v2";
const BB_KEY_NOTES = "bb_notes_v2";
const BB_KEY_ENC_TOKEN = "bb_enc_token_v2";   // AES-GCM encrypted token
const BB_KEY_GIST_ID_LS = "bb_gist_id";
const BB_KEY_SESSION = "bb_session_v2";
...
async function _ghSave() {
  ...
  try {
    ...
    if (!r.ok) { const e = await r.json(); return { ok: false, msg: e.message || r.status }; }
